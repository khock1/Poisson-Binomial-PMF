...
% probabilties of success in individual Bernoulli trials; values used as a demonstration example
success_probabilties=[0.25 0.15 0.05 0.1 0.8];

% number of trials
n_trials=length(success_probabilties);
...
for k = 1:n_trials
    ...
    for cmb = 1:size(this_k, 1)
        ...
        this_prod = 1;
        for i = 1:n_trials
            % if this trial is a success in this combination
            if ismember(i, this_comb)
                this_prod = this_prod * success_probabilties(i);
            else% else if this trial is a failure, use its probability of failure
                this_prod = this_prod * (1 - success_probabilties(i));
            end
        end
...
% unlike the probabilties of success in individual trials, all probabilities of obtaining 
% exactly k successes do need to add up to 1 , i.e., for all k when k = [0,..,n]
...
% obtaining exactly k sucesses given the success probabilties in individual
% independent trials/events?"
